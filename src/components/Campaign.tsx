...
import { useCampaignData, getCampaignData } from "@/hooks/useCampaignData";
...
const Campaign = () => {
  ...
  const { goal, collectedValue, collectedPercentValue, supporters, loading } = useCampaignData();
  
  const metaTotal = goal;
  const arrecadado = collectedValue;
  const percentual = collectedPercentValue;
  const apoiadores = supporters;
          ...
          >
            ...
            <div className="mb-8">
              <div className="flex justify-between items-end mb-3">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    {loading ? (
                      <span className="animate-pulse">arrecadados de R$ ---</span>
                    ) : (
                      `arrecadados de R$ ${metaTotal.toLocaleString('pt-BR')}`
                    )}
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2 mt-3 text-sm text-muted-foreground">
                ...
                {loading ? (
                  <span className="animate-pulse"><strong>--</strong> apoiadores</span>
                ) : (
                  <span><strong>{apoiadores}</strong> apoiadores</span>
                )}
              </div>
            </div>
